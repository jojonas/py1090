...
class Message:
	...
	Attributes:
		message_type (str): The message type: MSG, STA, ID, AIR, SEL or CLK. Usually only messages of type MSG (transmission messages)
			are of interest. Most of the software (e.g. :program:`dump1090`) does not even support any other message type.
		...
		transmission_type (int): Message subtype 1-8. Only used for transmission messages (MSG):
			...
			7        DF16            Air To Air Message (Triggered from TCAS - Traffic Alert and Collision Avoidance System)
			...
			Abbr.: DF = Downlink Format, BDS = Binary Data Store
		...
		ground_speed (int): Speed over ground (not indicated airspeed) in knots (use `helpers.knots_to_kmh` or `helpers.knots_to_mps` to convert).

		track (int): Track of aircraft (not heading), in degrees. Derived from the velocity E/W and velocity N/S.

		latitude (float): Latitude in degrees, North and East positive.

		longitude (float): Longitude in degrees, South and West negative.
	...
	def parse_string(self, string):
		...
		if parts[12]:
			self.ground_speed = int(parts[12])

		if parts[13]:
			self.track = int(parts[13])
